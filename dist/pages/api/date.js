...
var date_default = async (req, res) => {
  ...
  if (session) {
    if (req.method === "POST") {
      ...
      const requestDate = import_luxon.DateTime.fromISO(date).toJSDate();
          ...
          const product2 = await prisma.product.update({
            ...
            data: {
              date: {
                create: {
                  stock,
                  date: date ? requestDate : Date.now()
                }
              }
            }
          });
